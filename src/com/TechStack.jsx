...
import {
    ...
    SiFirebase, SiSupabase, SiDjango, SiMysql, SiGit, SiTypescript
} from 'react-icons/si';
...
export const TechStack = () => {
    ...
    const Tecknology = [
        ...
        { name: "TypeScript", icon: <SiTailwindcss className='text-blue-200' />, category: "Backend" },
        { name: 'MongoDB', icon: <SiMongodb className="text-green-500" />, category: 'Database' },
        { name: 'TypeScript', icon: <SiTypescript className="text-gray-800" />, category: 'Backend' },
    ...
    const filtersearch = Tecknology.filter(tech => tech.name.toLowerCase().includes(search.toLowerCase()) || tech.category.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        ...
        className="py-24 px-4 sm:px-6 bg-white">
            <h2 className='font-outfit text-center font-bold text-3xl sm:text-4xl lg:text-5xl text-gray-900 mb-6'>Tech Stack</h2>
            <p className="font-inter text-center text-base sm:text-lg lg:text-xl text-gray-600 max-w-2xl mx-auto mb-6">
                Technologies I use to build modern, scalable applications
            </p>

            <input type="text"
                placeholder='Search Technologies by name and caegory'
                className='w-full max-w-md mx-auto block px-4 py-2 border border-gray-700 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-gray-700 mb-8'
                value={search}
            ...
            <div 
            ref={cardRef}
            className='container mx-auto  grid grid-cols-1  lg:grid-cols-4 sm:grid-cols-2 gap-6  '>
                {filtersearch.map((tech, index) =>
                    <div
                        key={index}
                    ...
                    >
                        <div className='text-center'>
                            <div className='className="text-4xl sm:text-5xl mb-3 sm:mb-4 mx-auto p-3 rounded-full transition-transform duration-300 hover:scale-125 "
                  aria-hidden'>
                                {tech.icon}
